...
export default function TermsOfServiceScreen({ navigation }) {
  ...
  useEffect(() => {
    ...
    fetch(`${apiBase.replace(/\/$/, '')}/api/content/legal`)
      ...
      .then((data) => {
        if (cancelled) return;
        const text = data?.termsOfService && String(data.termsOfService).trim()
          ? String(data.termsOfService)
          : '';
        setContent(text);
      })
      .catch(() => {
    ...
    <View style={st.wrapper}>
      <View style={st.header}>
        <View style={st.headerRow}>
          ...
          <View style={st.hTitleBlock}>
            ...
            <Text style={st.hTitle}>TERMS OF SERVICE</Text>
          </View>
      ...
      >
        ...
        {loading ? (
          ...
          <Text style={st.emptyText}>Terms of Service content will be provided by the administrator.</Text>
        )}
